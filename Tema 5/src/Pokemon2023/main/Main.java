...
public class Main {
        ...
        for (Pokemon pokemon : pokemons) {
            System.out.println(
                    pokemon.getNombre() + " - " +
                            ...
                            pokemon.getPuntosSalud() + " puntos de salud"
            );
        }
            ...
            for (int i = 0; i < pokemons.length; i++) {
                ...
                if (atacante.estaVivo()) {
                    ...
                    try {
                        ...
                        System.out.println(
                                atacante.getNombre() + " ha atacado a " +
                                        objetivoPokemon.getNombre() + " - " +
                                        objetivoPokemon.getPuntosSalud() + " puntos de salud"
                        );
    ...
    private static Atacable seleccionaPokemonAtacable(Pokemon[] pokemons, int indiceAtacante) {
        ...
        for (int j = 0; j < pokemons.length; j++) {
            if (j != indiceAtacante &&
                    pokemons[j].estaVivo() &&
                    pokemons[j] instanceof Atacable) {

                contador++;
                System.out.println(
                        contador + ": " +
                                pokemons[j].getNombre() + " - " +
                                pokemons[j].getClass().getSimpleName() + " - " +
                                pokemons[j].getPuntosSalud() + " puntos de salud"
                );
        ...
        int opcion = UserDataCollector.getEnteroMinMax(
                "Selecciona un pokemon para atacar",
                1,
                contador
        );
        ...
        while (true) {
            try {
                ...
                return switch (random) {
                    case 0 -> new PokemonAgua(
                            generaNombrePokemonAleatorio(),
                            (int) (Math.random() * 51 + 50),
                            (int) (Math.random() * 11 + 5),
                            (int) (Math.random() * 21 + 5),
                            (int) (Math.random() * 11 + 10)
                    );
                    case 1 -> new PokemonFuego(
                            generaNombrePokemonAleatorio(),
                            (int) (Math.random() * 51 + 50),
                            (int) (Math.random() * 11 + 5),
                            (int) (Math.random() * 21 + 5),
    ...
    private static String generaNombrePokemonAleatorio() {
        String[] nombres = {
                ...
                "Meowth", "Psyduck", "Vulpix", "Gengar", "Gyarados",
                "Lapras", "Eevee", "Snorlax", "Articuno", "Zapdos",
                "Moltres", "Mewtwo", "Mew", "Chikorita", "Cyndaquil",
                "Totodile", "Togepi", "Ampharos", "Bellossom", "Marill",
                "Sudowoodo", "Unown", "Wobbuffet", "Girafarig", "Pineco",
                "Dunsparce", "Gligar", "Steelix", "Snubbull", "Qwilfish",
                "Scizor", "Shuckle", "Heracross", "Sneasel", "Teddiursa",
                "Ursaring", "Slugma", "Swinub", "Corsola", "Remoraid",
                "Delibird", "Mantine", "Skarmory", "Houndour", "Phanpy"
        };
